...
- `tools/gen_tcim_contract.py` 用 Clang AST 读取三个 GPU headers 和 `ds4.c` 的 local extern；`tcim/ds4_tcim_contract.json` 显式区分链接需求与实现/待实现/安全 fallback/范围外分类。`make regen-tcim-contract` 重建签名清单和精确 C stubs，`make check-tcim-contract` 检查漂移及 canonical link；`make test-tcim` 的链接检查仅依赖 Python/nm，hq50 无需 Clang。当前 287 个声明中有 24 个基础实现；237 个链接必需接口由 239 个唯一 strong provider 覆盖（含两个当前 CLI 未引用的基础 shim），这不是算子支持数。
